...
define([
    ...
    JXG.extend(JXG.CanvasRenderer.prototype, /** @lends JXG.CanvasRenderer.prototype */ {
        ...
        _setColor: function (element, type, targetType) {
            var hasColor = true, isTrace = false, ev = element.visProp, hl, rgba, rgbo, c, o, oo;
            type = type || 'stroke';
        ...
        drawPoint: function (el) {
            var f = el.visProp.face, size = el.visProp.size, scr = el.coords.scrCoords, sqrt32 = size * Math.sqrt(3) * 0.5, s05 = size * 0.5, stroke05 = parseFloat(el.visProp.strokewidth) / 2.0, context = this.context;
        ...
        drawLine: function (el) {
            var s, d, d1x, d1y, d2x, d2y, scr1 = new Coords(Const.COORDS_BY_USER, el.point1.coords.usrCoords, el.board), scr2 = new Coords(Const.COORDS_BY_USER, el.point2.coords.usrCoords, el.board), margin = null;
            if (!el.visProp.visible) {
                return;
            }
        ...
        updateTicks: function (ticks) {
            var i, c, x, y, len = ticks.ticks.length, context = this.context;
            context.beginPath();
            for (i = 0; i < len; i++) {
                c = ticks.ticks[i];
                x = c[0];
        ...
        setObjectStrokeColor: function (el, color, opacity) {
            var rgba = Type.evaluate(color), c, rgbo, o = Type.evaluate(opacity), oo, node;
            o = (o > 0) ? o : 0;
        ...
        updateImage: function (el) {
            var context = this.context, o = Type.evaluate(el.visProp.fillopacity), paintImg = Type.bind(function () {
                el.imgIsLoaded = true;
        ...
        transformImage: function (el, t) {
            var m, len = t.length, ctx = this.context;
            if (len > 0) {
        ...
        makeArrows: function (el, scr1, scr2) {
            ...
            */
            var x1, y1, x2, y2, ang, w = Math.max(el.visProp.strokewidth * 3, 10), arrowHead = [
                [-w, -w * 0.5],
                [0.0, 0.0],
                [-w, w * 0.5]
            ], arrowTail = [
        ...
        updatePathStringPrim: function (el) {
            var i, scr, scr1, scr2, len, symbm = 'M', symbl = 'L', symbc = 'C', nextSymb = symbm, maxSize = 5000.0, 
        ...
        updatePathStringBezierPrim: function (el) {
            var i, j, k, scr, lx, ly, len, symbm = 'M', symbl = 'C', nextSymb = symbm, maxSize = 5000.0, f = el.visProp.strokewidth, isNoPlot = (el.visProp.curvetype !== 'plot'), context = this.context;
            if (el.numberPoints <= 0) {
                return;
            }
