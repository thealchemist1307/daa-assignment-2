...
define([
    ...
    JXG.extend(JXG.Ticks.prototype, /** @lends JXG.Ticks.prototype */ {
        ...
        hasPoint: function (x, y) {
            var i, t, len = (this.ticks && this.ticks.length) || 0, r = this.board.options.precision.hasPoint;
        ...
        setPositionDirectly: function (method, coords, oldcoords) {
            var dx, dy, c = new Coords(method, coords, this.board), oldc = new Coords(method, oldcoords, this.board), bb = this.board.getBoundingBox();
            if (!this.line.visProp.scalable) {
                return this;
            }
            // horizontal line
        ...
        */
        calculateTicksCoordinates: function () {
            var coordsZero, bounds, i, oldRepoLength = this.labelsRepo.length;
        ...
        setTicksSizeVariables: function () {
            var d, distMaj = this.visProp.majorheight * 0.5, distMin = this.visProp.minorheight * 0.5;
            ...
            d = Math.sqrt(this.dxMaj * this.dxMaj * this.board.unitX * this.board.unitX +
        ...
        getLowerAndUpperBounds: function (coordsZero, type) {
            ...
            point1 = new Coords(Const.COORDS_BY_USER, this.line.point1.coords.usrCoords, this.board), point2 = new Coords(Const.COORDS_BY_USER, this.line.point2.coords.usrCoords, this.board), 
            ...
            isPoint1inBoard = (Math.abs(point1.usrCoords[0]) >= Mat.eps &&
                ...
                point1.scrCoords[2] >= 0.0 && point1.scrCoords[2] <= this.board.canvasHeight), isPoint2inBoard = (Math.abs(point2.usrCoords[0]) >= Mat.eps &&
        ...
        getDistanceFromZero: function (zero, point) {
            var eps = Mat.eps, distance = zero.distance(Const.COORDS_BY_USER, point);
            // Establish sign
            if (this.line.type === Const.OBJECT_TYPE_AXIS) {
        ...
        adjustTickDistance: function (ticksDelta, coordsZero, deltas) {
            var nx, ny, bounds, distScr, sgn = 1;
        ...
        generateFixedTicks: function (coordsZero, bounds) {
            var tickCoords, labelText, i, ti, x, y, hasLabelOverrides = Type.isArray(this.visProp.labels), 
            ...
            for (i = 0; i < this.fixedTicks.length; i++) {
                ...
                if (ti.length === 3 && this.fixedTicks[i] >= bounds.lower && this.fixedTicks[i] <= bounds.upper) {
                    ...
                    if (this.visProp.drawlabels && (hasLabelOverrides || Type.exists(this.visProp.labels[i]))) {
                        ...
                        this.labels.push(this.generateLabel(this.generateLabelText(tickCoords, coordsZero, labelText), tickCoords, i));
                    }
                    else {
                        this.labels.push(null);
                    }
        ...
        getXandYdeltas: function () {
            ...
            point1UsrCoords, point2UsrCoords, distP1P2 = this.line.point1.Dist(this.line.point2);
            if (this.line.type === Const.OBJECT_TYPE_AXIS) {
        ...
        tickEndings: function (coords, major) {
            var c, lineStdForm, intersection, dxs, dys, style, cw = this.board.canvasWidth, ch = this.board.canvasHeight, x = [-1000 * cw, -1000 * ch], y = [-1000 * cw, -1000 * ch], isInsideCanvas = false;
            c = coords.scrCoords;
            if (major) {
                dxs = this.dxMaj;
                dys = this.dyMaj;
        ...
        generateLabelText: function (tick, zero, value) {
            var labelText, distance = this.getDistanceFromZero(zero, tick);
        ...
        generateLabel: function (labelText, tick, tickNumber) {
            ...
            label.setCoords(tick.usrCoords[1] + label.distanceX / (this.board.unitX), tick.usrCoords[2] + label.distanceY / (this.board.unitY));
            return label;
        },
        /**
    ...
    JXG.createTicks = function (board, parents, attributes) {
        var el, dist, attr = Type.copyAttributes(attributes, board.options, 'ticks');
        if (parents.length < 2) {
    ...
    JXG.createHatchmark = function (board, parents, attributes) {
        var num, i, base, width, totalwidth, el, pos = [], attr = Type.copyAttributes(attributes, board.options, 'hatch');
