...
define([
    ...
    JXG.extend(JXG.Curve.prototype, /** @lends JXG.Curve.prototype */ {
        ...
        * @returns {Number} y-coordinate of the curve at t.
        ...
        hasPoint: function (x, y, start) {
            var t, checkPoint, len, invMat, c, i, j, tX, tY, res, points, qdt, steps = this.visProp.numberpointslow, d = (this.maxX() - this.minX()) / steps, prec = this.board.options.precision.hasPoint / this.board.unitX, dist = Infinity, suspendUpdate = true;
            ...
            if (this.visProp.curvetype === 'parameter' ||
                ...
                for (i = start; i < len; i++) {
                    ...
                    if (this.bezierDegree === 3) {
                        ...
                        if (qdt) {
                            if (points[i].prev) {
                                res.push(Geometry.projectCoordsToSegment([1, x, y], points[i].prev.usrCoords, points[i].usrCoords));
                            }
                            ...
                            if (points[i].next && points[i + 1] !== points[i].next) {
                                res.push(Geometry.projectCoordsToSegment([1, x, y], points[i].usrCoords, points[i].next.usrCoords));
                            }
                        }
                        else {
                            res.push(Geometry.projectCoordsToSegment([1, x, y], points[i].usrCoords, points[i + 1].usrCoords));
                        }
                    }
        ...
        updateTransformMatrix: function () {
            var t, c, i, len = this.transformations.length;
         ...
         **/
        checkReal: function () {
            var b = false, i, p, len = this.numberPoints;
            for (i = 0; i < len; i++) {
        ...
        updateParametricCurveOld: function (mi, ma) {
            var i, t, t0, d, x, y, x0, y0, top, depth, MAX_DEPTH, MAX_XDIST, MAX_YDIST, suspendUpdate = false, po = new Coords(Const.COORDS_BY_USER, [0, 0], this.board, false), dyadicStack = [], depthStack = [], pointStack = [], divisors = [], distOK = false, j = 0, distFromLine = function (p1, p2, p0) {
                var lbda, d, x0 = p0[1] - p1[1], y0 = p0[2] - p1[2], x1 = p2[0] - p1[1], y1 = p2[1] - p1[2], den = x1 * x1 + y1 * y1;
                if (den >= Mat.eps) {
        ...
        */
        ...
        _insertPoint: function (pnt) {
            ...
            cw = this.board.canvasWidth, ch = this.board.canvasHeight, off = 500;
            newReal = newReal &&
        ...
        _borderCase: function (a, b, c, ta, tb, tc, depth) {
            var t, pnt, p, p_good = null, j, max_it = 30, is_undef = false, t_nan, t_real, t_real2, box, vx, vy, vx2, vy2, dx, dy, x, y, asymptote, border, intersection;
            if (depth <= 1) {
        ...
        _isOutside: function (a, ta, b, tb) {
            var off = 500, cw = this.board.canvasWidth, ch = this.board.canvasHeight;
        ...
        _plotRecursive: function (a, ta, b, tb, depth, delta) {
            var tc, c, ds, mindepth = 0, isSmooth, isJump, isCusp, cusp_threshold = 0.5, pnt = new Coords(Const.COORDS_BY_USER, [0, 0], this.board, false);
        ...
        updateParametricCurve: function (mi, ma) {
            var ta, tb, a, b, suspendUpdate = false, pa = new Coords(Const.COORDS_BY_USER, [0, 0], this.board, false), pb = new Coords(Const.COORDS_BY_USER, [0, 0], this.board, false), depth, delta;
            //var stime = new Date();
            if (this.board.updateQuality === this.board.BOARD_QUALITY_LOW) {
                depth = 13;
                delta = 1.2;
        ...
        addTransform: function (transform) {
            var i, list = Type.isArray(transform) ? transform : [transform], len = list.length;
            for (i = 0; i < len; i++) {
        ...
        interpolationFunctionFromArray: function (which) {
            ...
            return function (t, suspendedUpdate) {
                var i, j, f1, f2, z, t0, t1, arr = this[data], len = arr.length, f = [];
                if (isNaN(t)) {
                    return NaN;
                }
                if (t < 0) {
        ...
        notifyParents: function (contentStr) {
            var fstr, dep, isJessieCode = false;
        ...
        bounds: function () {
            var minX = Infinity, maxX = -Infinity, minY = Infinity, maxY = -Infinity, l = this.points.length, i;
            if (this.bezierDegree === 3) {
                // Add methods X(), Y()
                for (i = 0; i < l; i++) {
    ...
    JXG.createTracecurve = function (board, parents, attributes) {
        ...
        c.updateDataArray = function () {
            var i, step, t, el, pEl, x, y, v, from, savetrace, le = attr.numberpoints, savePos = glider.position, slideObj = glider.slideObject, mi = slideObj.minX(), ma = slideObj.maxX();
            // set step width
            step = (ma - mi) / le;
            this.dataX = [];
            this.dataY = [];
    ...
    JXG.createStepfunction = function (board, parents, attributes) {
        ...
        c.updateDataArray = function () {
            var i, j = 0, len = this.xterm.length;
            this.dataX = [];
            this.dataY = [];
            if (len === 0) {
                return;
