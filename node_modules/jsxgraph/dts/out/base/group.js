...
define([
        ...
        update: function (drag) {
            ...
            if (drag.action === 'translation') {
                ...
                if (drag.action === 'rotation') {
                    ...
                    t = this.board.create('transform', [1, 0, 0,
                        center[0] * (1 - sx), sx, 0,
                        ...
                        if (obj.id !== drag.id) {
                            if (drag.action === 'translation') {
                                if (!Type.isInArray(drag.changed, obj.id)) {
                                    obj.coords.setCoordinates(Const.COORDS_BY_USER, [this.coords[el].usrCoords[1] + t[0],
                            ...
                            if (drag.action === 'rotation' || drag.action === 'scaling') {
                                obj.coords.setCoordinates(Const.COORDS_BY_USER, Mat.matVecMult(t.matrix, this.coords[obj.id].usrCoords));
                            }
                        }
                    }
                    else {
    ...
    JXG.createGroup = function (board, parents, attributes) {
        var attr = Type.copyAttributes(attributes, board.options, 'group'), g = new JXG.Group(board, attr.id, attr.name, parents, attr);
        g.elType = 'group';
