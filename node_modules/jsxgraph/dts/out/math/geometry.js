...
define([
    ...
    JXG.extend(Mat.Geometry, /** @lends JXG.Math.Geometry */ {
        ...
        sortVertices: function (p) {
            var i, ll, ps = Expect.each(p, Expect.coordsArray), N = ps.length;
            ...
            ps.sort(function (a, b) {
                ...
                var rad1 = Math.atan2(a[2] - ll[2], a[1] - ll[1]), rad2 = Math.atan2(b[2] - ll[2], b[1] - ll[1]);
                return rad1 - rad2;
            });
        ...
        GrahamScan: function (points) {
            var i, M = 1, ps = Expect.each(points, Expect.coordsArray), N = ps.length;
            ps = this.sortVertices(ps);
            N = ps.length;
            for (i = 2; i < N; i++) {
        ...
        intersectionFunction: function (board, el1, el2, i, j, alwaysintersect) {
            ...
            if (el1.elementClass === Const.OBJECT_CLASS_CURVE &&
                ...
                /** @ignore */
                func = function () {
                    var res, c, first1 = el1.visProp.straightfirst, first2 = el2.visProp.straightfirst, last1 = el1.visProp.straightlast, last2 = el2.visProp.straightlast;
                    /**
                    ...
                    if (!alwaysintersect && (!first1 || !last1 || !first2 || !last2)) {
                        res = that.meetSegmentSegment(el1.point1.coords.usrCoords, el1.point2.coords.usrCoords, el2.point1.coords.usrCoords, el2.point2.coords.usrCoords, el1.board);
        ...
        meetCurveLineContinuous: function (cu, li, nr, board, testSegment) {
            var t, func0, func1, func0a, v, x, y, z, eps = Mat.eps, epsLow = Mat.eps, steps, delta, tnew, i, tmin, fmin;
            v = this.meetCurveLineDiscrete(cu, li, nr, board, testSegment);
            x = v.usrCoords[1];
            y = v.usrCoords[2];
            func0 = function (t) {
                var c1 = x - cu.X(t), c2 = y - cu.Y(t);
                return c1 * c1 + c2 * c2;
            };
            func1 = function (t) {
        ...
        meetCurveLineContinuousOld: function (cu, li, nr, board) {
            var t, t2, i, func, z, tnew, steps, delta, tstart, tend, cux, cuy, eps = Mat.eps * 10;
        ...
        meetCurveLineDiscrete: function (cu, li, nr, board, testSegment) {
            var i, j, p1, p2, p, q, lip1 = li.point1.coords.usrCoords, lip2 = li.point2.coords.usrCoords, d, res, cnt = 0, len = cu.numberPoints;
        ...
        meetCurveRedBlueSegments: function (red, blue, nr) {
            var i, j, red1, red2, blue1, blue2, m, minX, maxX, iFound = 0, lenBlue = blue.numberPoints, //points.length,
        ...
        meetSegmentSegment: function (p1, p2, q1, q2) {
            var t, u, diff, li1 = Mat.crossProduct(p1, p2), li2 = Mat.crossProduct(q1, q2), c = Mat.crossProduct(li1, li2), denom = c[0];
        ...
        meetBezierCurveRedBlueSegments: function (red, blue, nr) {
            var p, i, j, redArr, blueArr, bbr, bbb, lenBlue = blue.numberPoints, //points.length,
            lenRed = red.numberPoints, // points.length,
            L = [];
        ...
        projectPointToCurve: function (point, curve, board) {
            ...
            var x = point.X(), y = point.Y(), t = point.position || 0.0, result = this.projectCoordsToCurve(x, y, t, curve, board);
        ...
        projectCoordsToCurve: function (x, y, t, curve, board) {
            var newCoords, newCoordsObj, i, j, mindist, dist, lbda, v, coords, d, p1, p2, res, minfunc, tnew, fnew, fold, delta, steps, infty = Number.POSITIVE_INFINITY;
            if (!Type.exists(board)) {
                board = curve.board;
            }
            if (curve.visProp.curvetype === 'plot') {
                ...
                minfunc = function (t) {
                    var dx = x - curve.X(t), dy = y - curve.Y(t);
                    return dx * dx + dy * dy;
                };
                fold = minfunc(t);
                steps = 50;
        ...
        projectCoordsToPolygon: function (p, pol) {
            var i, len = pol.vertices.length, d_best = Infinity, d, projection, bestprojection;
            for (i = 0; i < len; i++) {
                projection = JXG.Math.Geometry.projectCoordsToSegment(p, pol.vertices[i].coords.usrCoords, pol.vertices[(i + 1) % len].coords.usrCoords);
        ...
        projectPointToTurtle: function (point, turtle, board) {
            var newCoords, t, x, y, i, dist, el, minEl, np = 0, npmin = 0, mindist = Number.POSITIVE_INFINITY, len = turtle.objects.length;
            if (!Type.exists(board)) {
                board = point.board;
            }
