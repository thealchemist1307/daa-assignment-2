...
JXG.Board.prototype.construct = function (string, mode, params, paraIn, macroName) {
    var splitted, i, j, output = {}, objName, defElements, obj, type, possibleNames, tmp, noMacro, k, l, pattern, createdNames, found, mac, prop, propName, propValue, attributes;
    if (!JXG.exists(mode)) {
        mode = "normal";
    }
    else {
                ...
                if (!prop) {
                    ...
                    if (noMacro) {
                        if (splitted[i].search(/^[\[\]].*[\[\]]$/) != -1) {
                            ...
                            output.points.push(this.create('glider', [defElements[1], defElements[2], JXG.getReference(this, defElements[0])], attributes));
                            output[objName] = output.points[output.points.length - 1];
                        }
                            ...
                            output.lines.push(this.create(type, [JXG.getReference(this, defElements[0]), JXG.getReference(this, defElements[1])], attributes));
                            if (objName != '') {
                                output[objName] = output.lines[output.lines.length - 1];
                            }
                        }
                            ...
                            if (objName == '') {
                                output.lines.push(this.create('angle', [JXG.getReference(this, defElements[0]),
                                    JXG.getReference(this, defElements[1]),
                                    JXG.getReference(this, defElements[2])], attributes));
                            }
                            else {
                                ...
                                output.angles.push(this.create('angle', [JXG.getReference(this, defElements[0]),
                                    JXG.getReference(this, defElements[1]),
                                    JXG.getReference(this, defElements[2])], attributes));
                                output[objName] = output.angles[output.angles.length - 1];
                            }
                        }
                        else if (splitted[i].search(/([0-9]+)\/([0-9]+)\(\s*(\S*)\s*,\s*(\S*)\s*\)/) != -1) {
                            defElements = [];
                        ...
                        else if (splitted[i].search(/#(.*)\(\s*([0-9])\s*[,|]\s*([0-9])\s*\)/) != -1) {
                            defElements = []; // [0-9\.\-]+
                            defElements[0] = RegExp.$1;
                            defElements[1] = 1.0 * RegExp.$2;
                            defElements[2] = 1.0 * RegExp.$3;
