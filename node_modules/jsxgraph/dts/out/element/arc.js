...
define([
    ...
    JXG.createArc = function (board, parents, attributes) {
        ...
        el.updateDataArray = function () {
            var ar, phi, v, det, p0c, p1c, p2c, sgn = 1, A = this.radiuspoint, B = this.center, C = this.anglepoint;
        ...
        el.hasPoint = function (x, y) {
            var dist, checkPoint, has, angle, alpha, beta, invMat, c, prec = this.board.options.precision.hasPoint / this.board.unitX, r = this.Radius();
        ...
        el.hasPointSector = function (x, y) {
            var angle, alpha, beta, checkPoint = new Coords(Const.COORDS_BY_SCREEN, [x, y], this.board), r = this.Radius(), dist = this.center.coords.distance(Const.COORDS_BY_USER, checkPoint), has = (dist < r);
            if (has) {
                angle = Geometry.rad(this.radiuspoint, this.center, checkPoint.usrCoords.slice(1));
                alpha = 0;
                beta = Geometry.rad(this.radiuspoint, this.center, this.anglepoint);
        ...
        el.getLabelAnchor = function () {
            var coords, vecx, vecy, len, angle = Geometry.rad(this.radiuspoint, this.center, this.anglepoint), dx = 10 / this.board.unitX, dy = 10 / this.board.unitY, p2c = this.point2.coords.usrCoords, pmc = this.center.coords.usrCoords, bxminusax = p2c[1] - pmc[1], byminusay = p2c[2] - pmc[2];
