...
define([
    ...
    JXG.createSector = function (board, parents, attributes) {
        var el, i, attr, type = 'invalid', s, v, attrPoints = ['center', 'radiuspoint', 'anglepoint'], points;
        // Three points?
        ...
        if (type === '2lines') {
            ...
            */
            el.updateDataArray = function () {
                var ar, det, p0c, p1c, p2c, A = this.point2, B = this.point1, C = this.point3, phi, sgn = 1;
        ...
        el.hasPointCurve = function (x, y) {
            var angle, alpha, beta, prec = this.board.options.precision.hasPoint / (this.board.unitX), checkPoint = new Coords(Const.COORDS_BY_SCREEN, [x, y], this.board), r = this.Radius(), dist = this.center.coords.distance(Const.COORDS_BY_USER, checkPoint), has = (Math.abs(dist - r) < prec);
            if (has) {
        ...
        */
        el.hasPointSector = function (x, y) {
            var angle, checkPoint = new Coords(Const.COORDS_BY_SCREEN, [x, y], this.board), r = this.Radius(), dist = this.point1.coords.distance(Const.COORDS_BY_USER, checkPoint), alpha, beta, has = (dist < r);
            if (has) {
        ...
        el.getLabelAnchor = function () {
            var coords, vecx, vecy, len, angle = Geometry.rad(this.point2, this.point1, this.point3), dx = 13 / this.board.unitX, dy = 13 / this.board.unitY, p2c = this.point2.coords.usrCoords, pmc = this.point1.coords.usrCoords, bxminusax = p2c[1] - pmc[1], byminusay = p2c[2] - pmc[2];
        ...
        if (type === '3points') {
            el.setPositionDirectly = function (method, coords, oldcoords) {
                var dc, t, i, len, c = new Coords(method, coords, this.board), oldc = new Coords(method, oldcoords, this.board);
 ...
 * })();
    ...
    JXG.createAngle = function (board, parents, attributes) {
        var el, radius, text, attr, attrsub, i, dot, points, type = 'invalid';
        ...
        if (type === '2lines') {
            ...
            el.updateDataArraySector = function () {
                var A = this.point2, B = this.point1, C = this.point3, r = this.Radius(), d = B.Dist(A), ar, phi, sgn = 1;
                phi = Geometry.rad(A, B, C);
                if ((this.visProp.selection === 'minor' && phi > Math.PI) ||
                    (this.visProp.selection === 'major' && phi < Math.PI)) {
                    sgn = -1;
            ...
            */
            el.setAngle = function (val) {
                var t, p = this.anglepoint, q = this.radiuspoint;
                if (p.draggable()) {
        ...
        el.updateDataArraySquare = function () {
            var A, B, C, r = this.Radius(), d1, d2, v, l1, l2;
            if (type === '2lines') {
        ...
        el.updateDataArray = function () {
            var type = this.visProp.type, deg = Geometry.trueAngle(this.point2, this.point1, this.point3);
        ...
        el.dot = board.create('point', [function () {
                var A, B, r, d, a2, co, si, mat, point1, point2, point3;
        ...
        // documented in GeometryElement
        el.getLabelAnchor = function () {
            var vec, dx = 12, dy = 12, A, B, r, d, a2, co, si, mat;
            // If this is uncommented, the angle label can not be dragged
            //if (Type.exists(this.label)) {
            //    this.label.relativeCoords = new Coords(Const.COORDS_BY_SCREEN, [0, 0], this.board);
            //}
