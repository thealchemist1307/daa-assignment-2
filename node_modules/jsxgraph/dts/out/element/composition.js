...
define([
    ...
    JXG.createOrthogonalProjection = function (board, parents, attributes) {
        ...
        t.generatePolynomial = function () {
             ...
             *
             */
            var a1 = l.point1.symbolic.x, a2 = l.point1.symbolic.y, b1 = l.point2.symbolic.x, b2 = l.point2.symbolic.y, p1 = p.symbolic.x, p2 = p.symbolic.y, t1 = t.symbolic.x, t2 = t.symbolic.y, poly1 = '(' + a2 + ')*(' + t1 + ')-(' + a2 + ')*(' + b1 + ')+(' + t2 + ')*(' + b1 + ')-(' +
                a1 + ')*(' + t2 + ')+(' + a1 + ')*(' + b2 + ')-(' + t1 + ')*(' + b2 + ')', poly2 = '(' + p2 + ')*(' + a2 + ')-(' + p2 + ')*(' + b2 + ')-(' + t2 + ')*(' + a2 + ')+(' +
    ...
    JXG.createMidpoint = function (board, parents, attributes) {
        ...
        t.generatePolynomial = function () {
             ...
             *
             */
            var a1 = a.symbolic.x, a2 = a.symbolic.y, b1 = b.symbolic.x, b2 = b.symbolic.y, t1 = t.symbolic.x, t2 = t.symbolic.y, poly1 = '(' + a2 + ')*(' + t1 + ')-(' + a2 + ')*(' + b1 + ')+(' + t2 + ')*(' + b1 + ')-(' +
                a1 + ')*(' + t2 + ')+(' + a1 + ')*(' + b2 + ')-(' + t1 + ')*(' + b2 + ')', poly2 = '(' + a1 + ')^2 - 2*(' + a1 + ')*(' + t1 + ')+(' + a2 + ')^2-2*(' + a2 + ')*(' +
    ...
    JXG.createParallelPoint = function (board, parents, attributes) {
        ...
        p.generatePolynomial = function () {
             ...
             *
             */
            var a1 = a.symbolic.x, a2 = a.symbolic.y, b1 = b.symbolic.x, b2 = b.symbolic.y, c1 = c.symbolic.x, c2 = c.symbolic.y, t1 = p.symbolic.x, t2 = p.symbolic.y, poly1 = '(' + b2 + ')*(' + t1 + ')-(' + b2 + ')*(' + c1 + ')-(' + a2 + ')*(' + t1 + ')+(' +
                a2 + ')*(' + c1 + ')-(' + t2 + ')*(' + b1 + ')+(' + t2 + ')*(' + a1 + ')+(' + c2 + ')*(' +
                b1 + ')-(' + c2 + ')*(' + a1 + ')', poly2 = '(' + t2 + ')*(' + a1 + ')-(' + t2 + ')*(' + c1 + ')-(' + b2 + ')*(' + a1 + ')+(' +
    ...
    JXG.createNormal = function (board, parents, attributes) {
        ...
        if (c.elementClass === Const.OBJECT_CLASS_LINE) {
            ...
            if (c.visProp.curvetype !== 'plot') {
                ...
                l = board.create('line', [
                    function () {
                        var i = Math.floor(p.position), lbda = p.position - i;
            ...
            l = board.create('line', [
                function () {
                    var el, j, i = Math.floor(p.position), lbda = p.position - i;
                ...
                function () {
                    var el, j, i = Math.floor(p.position);
                    // run through all curves of this turtle
                    for (j = 0; j < c.objects.length; j++) {
                        el = c.objects[j];
                        if (el.type === Const.OBJECT_TYPE_CURVE) {
    ...
    JXG.createAngularBisectorsOfTwoLines = function (board, parents, attributes) {
        ...
        g1 = board.create('line', [
            function () {
                var d1 = Math.sqrt(l1.stdform[1] * l1.stdform[1] + l1.stdform[2] * l1.stdform[2]), d2 = Math.sqrt(l2.stdform[1] * l2.stdform[1] + l2.stdform[2] * l2.stdform[2]);
    ...
    JXG.createIncircle = function (board, parents, attributes) {
        ...
        try {
            ...
            c = Circle.createCircle(board, [p, function () {
                    var a = Math.sqrt((parents[1].X() - parents[2].X()) * (parents[1].X() - parents[2].X()) + (parents[1].Y() - parents[2].Y()) * (parents[1].Y() - parents[2].Y())), b = Math.sqrt((parents[0].X() - parents[2].X()) * (parents[0].X() - parents[2].X()) + (parents[0].Y() - parents[2].Y()) * (parents[0].Y() - parents[2].Y())), c = Math.sqrt((parents[1].X() - parents[0].X()) * (parents[1].X() - parents[0].X()) + (parents[1].Y() - parents[0].Y()) * (parents[1].Y() - parents[0].Y())), s = (a + b + c) / 2;
    ...
    JXG.createReflection = function (board, parents, attributes) {
        ...
        r.generatePolynomial = function () {
             ...
             *
             */
            var a1 = l.point1.symbolic.x, a2 = l.point1.symbolic.y, b1 = l.point2.symbolic.x, b2 = l.point2.symbolic.y, p1 = p.symbolic.x, p2 = p.symbolic.y, r1 = r.symbolic.x, r2 = r.symbolic.y, poly1 = ['((', r2, ')-(', p2, '))*((', a2, ')-(', b2, '))+((', a1, ')-(', b1, '))*((', r1, ')-(', p1, '))'].join(''), poly2 = ['((', r1, ')-(', a1, '))^2+((', r2, ')-(', a2, '))^2-((', p1, ')-(', a1, '))^2-((', p2, ')-(', a2, '))^2'].join('');
            return [poly1, poly2];
        };
        return r;
    };
    ...
    JXG.createIntegral = function (board, parents, attributes) {
        var interval, curve, attr, start, end, startx, starty, endx, endy, pa_on_curve, pa_on_axis, pb_on_curve, pb_on_axis, t = null, p;
    ...
    JXG.createGrid = function (board, parents, attributes) {
        ...
        c.updateDataArray = function () {
            var start, end, i, topLeft, bottomRight, gridX = this.visProp.gridx, gridY = this.visProp.gridy;
    ...
    JXG.createInequality = function (board, parents, attributes) {
        ...
        if (parents[0].elementClass === Const.OBJECT_CLASS_LINE) {
            ...
            a.updateDataArray = function () {
                ...
                h, bb = board.getBoundingBox(), factor = attr.inverse ? -1 : 1, expansion = 1.5, w = expansion * Math.max(bb[2] - bb[0], bb[1] - bb[3]), 
