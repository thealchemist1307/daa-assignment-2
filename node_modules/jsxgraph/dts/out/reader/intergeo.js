...
(function () {
    ...
    JXG.extend(JXG.IntergeoReader.prototype, /** @lends JXG.IntergeoReader.prototype */ {
        ...
        storePoint: function (node) {
            var i = 0, j = 0, l = 0, p = node.childNodes[i], c = [], parents = [];
         ...
         */
        addLine: function (id) {
            var j, c = this.objects[id].coords, el;
            for (j = 0; j < c.length; j++) {
                c[j] = parseFloat(c[j]);
            }
        ...
        addLineThroughTwoPoints: function (node, isRay) {
            var el1, el2, el, param = this.readParams(node);
        ...
        addLineThroughPoint: function (node) {
            var param = this.readParams(node), j, c = this.objects[param[0]].coords, p = this.addPoint(this.objects[param[1]]), el;
            for (j = 0; j < c.length; j++) {
                c[j] = parseFloat(c[j]);
            }
        ...
        addPointIntersectionOfTwoLines: function (node) {
            var param = this.readParams(node), l1 = this.objects[param[1]], l2 = this.objects[param[2]];
        ...
        addFreeLine: function (node) {
            var param = this.readParams(node), a = this.objects[param[0]].coords[0], b = this.objects[param[0]].coords[1], c = this.objects[param[0]].coords[2];
        ...
        addPointOnLine: function (node) {
            var param = this.readParams(node), l = this.board.select(param[1]);
            this.board.create('glider', [0, 0, l], { name: param[0], id: param[0], withLabel: true });
            this.objects[param[0]].exists = true;
        },
        addPointOnCircle: function (node) {
            var param = this.readParams(node), c = this.board.select(param[1]);
            c.update();
        ...
        addEndpointsOfLineSegment: function (node) {
            var param = this.readParams(node), line = this.objects[param[2]], p = this.addPoint(this.objects[param[0]]), q = this.addPoint(this.objects[param[1]]);
            p.addConstraint([
                function () {
        ...
        addAngularBisectorOfThreePoints: function (node, isLine) {
            var el1, el2, el3, el, param = this.readParams(node);
            el1 = this.addPoint(this.objects[param[1]]);
            el2 = this.addPoint(this.objects[param[2]]);
        ...
        addMidpointOfLineSegment: function (node) {
            var param = this.readParams(node), l = this.board.select(param[1]), el = this.board.create('midpoint', [l.point1, l.point2], { name: param[0] });
            this.setAttributes(el);
            this.objects[param[0]].exists = true;
        },
        addCircleByThreePoints: function (node) {
            var i, param = this.readParams(node), p = [];
            for (i = 0; i < 3; i++) {
        ...
        addCenterOfCircle: function (node) {
            var param = this.readParams(node), c = this.board.select(param[1]), el = this.board.create('point', [
                function () {
                    return c.center.X();
                },
                function () {
        ...
        addCircleTangentLinesByPoint: function (node) {
            var polar, i1, i2, t1, t2, param = this.readParams(node), c = this.board.select(param[2]), p = this.addPoint(this.objects[param[3]]);
        ...
        addIntersectionPointsOfTwoCircles: function (node) {
            var p1, p2, param = this.readParams(node), c1 = this.board.select(param[2]), c2 = this.board.select(param[3]);
        ...
        addCircleByCenterAndPoint: function (node) {
            var param = this.readParams(node), el1 = this.addPoint(this.objects[param[1]]), el2 = this.addPoint(this.objects[param[2]]);
        ...
        addOtherIntersectionPointOfTwoCircles: function (node) {
            var param = this.readParams(node), c1 = this.board.select(param[2]), c2 = this.board.select(param[3]), p1 = this.board.select(param[1]), // Should exist by now
            p2;
        ...
        addAngularBisectorsOfTwoLines: function (node) {
            var param = this.readParams(node), l1 = this.objects[param[2]], l2 = this.objects[param[3]];
        ...
        addPolygonByVertices: function (node) {
            var j, n, param2 = [], p = [], param = this.readParams(node);
            for (j = 0; j < node.childNodes.length; j++) {
                if (node.childNodes[j].nodeType === 1) {
        ...
        addVectorFromPointToPoint: function (node) {
            var el, el1, el2, param = this.readParams(node);
            el1 = this.addPoint(this.objects[param[1]]);
            el2 = this.addPoint(this.objects[param[2]]);
        ...
        addLocusDefinedByPoint: function (node) {
            var param = this.readParams(node), el = this.board.select(param[1]);
