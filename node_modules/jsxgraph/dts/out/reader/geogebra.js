...
(function () {
    ...
    JXG.extend(JXG.GeogebraReader.prototype, /** @lends JXG.GeogebraReader.prototype */ {
        ...
        ggbAct: function (type, m, n) {
            var s1, s2, a, regexValue = new RegExp('JXG\\.boards\\[\'' + this.board.id + '\'\\].select\\(\"(.+?)\"\\)\\.'), regexSelect = new RegExp('JXG\\.boards\\[\'' + this.board.id + '\'\\].select'), v1 = m, v2 = n;
        ...
        ggbParse: function (exp, el) {
            var i, error_count, error_offsets = [], error_lookaheads = [], errstr = '', str = exp, dbg_withtrace = false, dbg_string = '', ggbr = this, element = el ? this.board.select(this.ggbElements[el].id) : false;
            if (element) {
            ...
            function lex(info) {
                var state = 0, match = -1, match_pos = 0, start = 0, pos = info.offset + 1;
                do {
                    pos -= 1;
                    state = 0;
                    match = -2;
            ...
            function parse(src, err_off, err_la) {
                var i, act, go, la, rval, rvstack, rsstack, undef, act_tab, pop_tab, goto_tab, labels, sstack = [], vstack = [], err_cnt = 0, info = {};
                // Pop-Table
                pop_tab = [
                    [0, 1],
                    [27, 1],
                ...
                act_tab = [
                    /* State 0 */
                    [2, 3, 23, 4, 12, 5, 9, 6, 4, 7, 5, 8, 6, 9, 8, 10, 10, 11, 7, 12],
                    /* State 1 */
                    [28, 0],
                    /* State 2 */
        ...
        colorProperties: function (Data, attr) {
            var a, r, g, b, objColor = Data.getElementsByTagName("objColor")[0], len = Data.getElementsByTagName("objColor").length > 0;
        ...
        writeBoard: function () {
            ...
            boardData = this.tree.getElementsByTagName("euclidianView")[0], coordSystem = boardData.getElementsByTagName('coordSystem')[0], gui = this.tree.getElementsByTagName('gui')[0], evSettings = boardData.getElementsByTagName('evSettings')[0];
            this.board.origin = {};
        ...
        writeElement: function (output, input, cmd) {
            var p, res, re2, poly, t2, t, m, i, l2, p2, l1, p1, slopeWidth, tmp, attr2, t1, i2, i1, pol, type, d2, d1, d, startpoint, inp, borderatts, borders, element, gxtEl, attr, exp, coord, points, length, match, matchDep, rx, q, c, s, e, sx, sy, ex, ey, func, range, 
            // for use with closures
            that = this, makeConstFun = function (a) {
                return function () {
                    return a;
                };
                        ...
                        p = this.board.create('text', [
                            ...
                            function () {
                                return "<span style='text-decoration: overline'>" + input[0].name + input[1].name + "</span> = " +
                                    JXG.trimNumber(JXG.toFixed(that.board.select(input[0].id).Dist(that.board.select(input[1].id)), that.decimals));
                            }
                        ], attr);
                        p.Value = function () {
                ...
                case 'slope':
                    ...
                    try {
                        ...
                        t = this.board.create('text', [
                            ...
                            function () {
                                return "&nbsp;&nbsp;" + (slopeWidth > 1 ? slopeWidth.toString() : '') + ' ' + this.name + ' = ' + JXG.trimNumber(JXG.toFixed(slopeWidth * input[0].getSlope(), that.decimals));
                            }
                        ], attr);
                        attr.name = '';
        ...
        prepareString: function (fileStr) {
            var i, bA, len, fstr, isString = fileStr.slice(0, 2) !== "PK";
